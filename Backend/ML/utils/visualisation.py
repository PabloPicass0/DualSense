...
def plotWrongImages(X_test, y_test, y_pred, n_img, class_names):
    ...
    for index, ax in zip(indices, axes):
        ...
        rect = patches.Rectangle((0, 0), X_test.shape[2] - 1, X_test.shape[1] - 1, linewidth=1, edgecolor='black', facecolor='none')
        ax.add_patch(rect)
...
def plotWrongImagesWithCharts(X_test, y_test, y_pred, reconstructed_images, n_img, class_names):
    ...
    for i, index in enumerate(indices[:n_img]):
        ...
        ax.bar(range(len(class_names)), lengths, color=['g' if i == np.argmax(y_test[index]) else 'r' for i in range(len(class_names))])
        ...
        ax.axis('off')
        rect = patches.Rectangle((0, 0), reconstructed_images.shape[2]-1, reconstructed_images.shape[1]-1, linewidth=1, edgecolor='black', facecolor='none')
        ax.add_patch(rect)
